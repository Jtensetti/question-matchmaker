  ...
  const handleSubmitAnswer = async () => {
    if (questionType === "rating") {
      ...
      if (isExactlyCorrect) {
        toast({
          ...
          description: "You selected the correct value!",
        });
      } else {
        toast({
          title: "Incorrect answer",
          description: `Correct value is: ${correctValue}`,
          variant: "destructive",
        });
      }
    ...
    if (questionType === "multiple-choice") {
      ...
      if (isExactlyCorrect) {
        toast({
          title: "Correct answer!",
          description: "You selected the correct option!",
        });
      } else {
        toast({
          title: "Incorrect answer",
          description: `Correct option is: ${question.answer}`,
          variant: "destructive",
        });
      }
    ...
    if (questionType === "checkbox") {
      ...
      if (isExactlyCorrect) {
        toast({
          title: "Correct answer!",
          description: "You selected all correct options!",
        });
      } else {
        toast({
          title: "Incorrect answer",
          description: `Correct options are: ${question.answer}`,
          variant: "destructive",
        });
      }
    ...
    if (questionType === "grid") {
      if (Object.keys(gridAnswers).length === 0) {
        toast({
          title: "Empty answer",
          description: "Please fill in at least one cell in the table",
          variant: "destructive",
        });
        return;
      }
      ...
      toast({
        ...
        description: "Your teacher will evaluate your answer",
      });
    ...
    if (!userAnswer.trim()) {
      toast({
        title: "Empty answer",
        description: "Write a response before submitting",
        variant: "destructive",
      });
      return;
    }
      ...
      return (
        <div className="space-y-2 mt-4 p-3 border border-amber-300 bg-amber-50 rounded-md">
          <div className="flex items-center gap-2 text-amber-600">
            ...
            <p className="text-sm">This question type is missing the correct configuration.</p>
          </div>
          <Input
            value={userAnswer}
      ...
      <div className="space-y-4 mt-4">
        ...
        {ratingValue !== null && !isSubmitted && (
          <div className="text-center text-sm">
            Your value: <span className="font-semibold">{ratingValue}</span>
          </div>
        )}
      </div>
    );
    ...
    if (!question.options || question.options.length === 0) {
      ...
      return (
        <div className="space-y-2 mt-4 p-3 border border-amber-300 bg-amber-50 rounded-md">
          <div className="flex items-center gap-2 text-amber-600">
            <Info className="h-4 w-4" />
            <p className="text-sm">This multiple choice question is missing options.</p>
          </div>
          <Input
            value={userAnswer}
            onChange={e => setUserAnswer(e.target.value)}
    ...
    if (!question.options || question.options.length === 0) {
      ...
      return (
        <div className="space-y-2 mt-4 p-3 border border-amber-300 bg-amber-50 rounded-md">
          <div className="flex items-center gap-2 text-amber-600">
            <Info className="h-4 w-4" />
            <p className="text-sm">This checkbox question is missing options.</p>
          </div>
          <Input
            value={userAnswer}
            onChange={e => setUserAnswer(e.target.value)}
    ...
    if (!question.gridRows || !question.gridColumns || 
      ...
      return (
        <div className="space-y-2 mt-4 p-3 border border-amber-300 bg-amber-50 rounded-md">
          <div className="flex items-center gap-2 text-amber-600">
            <Info className="h-4 w-4" />
            <p className="text-sm">This grid question is missing the correct configuration.</p>
          </div>
          <Input
            value={userAnswer}
            onChange={e => setUserAnswer(e.target.value)}
      ...
      <CardContent className="pt-6">
        <div className="space-y-4">
          <div>
            ...
            {(isTeacher || isAdmin) && questionType === "rating" && (
              <p className="text-sm text-muted-foreground mt-1">
                Correct value: {question.answer} (Min: {question.ratingMin}, Max: {question.ratingMax})
              </p>
            )}
            
            {(isTeacher || isAdmin) && (
              <div className="flex items-center mt-2 text-xs text-muted-foreground">
                <span className="mr-2">Question type: {questionType || "text"}</span>
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-[300px] p-4">
                      <p className="font-semibold mb-1">Question details:</p>
                      <ul className="list-disc pl-4 space-y-1">
                        <li>ID: {question.id}</li>
                        <li>Type: {questionType || "text"}</li>
                        <li>Raw type: {JSON.stringify(question.questionType)}</li>
                        {question.options && <li>Options: {question.options.join(", ")}</li>}
                        {question.ratingMin && <li>Min: {question.ratingMin}</li>}
                        {question.ratingMax && <li>Max: {question.ratingMax}</li>}
                        {question.gridRows && <li>Rows: {question.gridRows.length}</li>}
                        {question.gridColumns && <li>Columns: {question.gridColumns.length}</li>}
                      </ul>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
